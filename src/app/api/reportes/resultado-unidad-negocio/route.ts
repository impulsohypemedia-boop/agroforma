...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[resultado-unidad-negocio] extractedData: ${extractedData?.length ?? "null"}, textos: ${textos_extraidos ? Object.keys(textos_extraidos).length + " files" : "null"}`);
    ...
    userContent = parts.join("\n\n---\n\n") + `\n\nUsá TODA la información disponible arriba. Generá el JSON del Resultado por Unidad de Negocio.`;
    console.log(`[resultado-unidad-negocio] prompt length: ${userContent.length} (mode: ${extractedData?.length > 0 ? "structured" : "raw_texts"})`);
    ...
    console.error("Error en /api/reportes/resultado-unidad-negocio:", err);
